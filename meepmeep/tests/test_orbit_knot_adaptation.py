"""Tests for the eccentricity-adaptive knot grid in the Orbit class.

The ``'ea'`` / ``'ta'`` knot-placement strategies cluster knots near
...
than ``Orbit._KNOT_GRID_E_TOL`` from the eccentricity the current grid
was built for. Rebuilds are deliberately rare: ``create_knots`` runs
...
class TestKnotGridAdaptation:
    def test_grid_adapts_to_high_e(self):
        """After binding a high eccentricity, the knot grid must match the
        one create_knots builds for that eccentricity."""
        o = Orbit(npt=15)
        o.set_pars(tc=0.0, **HIGH_E_PARS)
        expected, _, _, _ = create_knots(15, HIGH_E_PARS["e"], "ea")
        assert_allclose(o._points, expected)
    ...
    def test_rebuild_beyond_tolerance(self):
        ...
        expected, _, _, _ = create_knots(15, 0.50, "ea")
        assert_allclose(o._points, expected)
    ...
    def test_mm_grid_never_rebuilt(self):
        ...
        o = Orbit(npt=15, knot_placement="mm")
        pts_init = o._points
        o.set_pars(tc=0.0, **HIGH_E_PARS)
        assert o._points is pts_init
