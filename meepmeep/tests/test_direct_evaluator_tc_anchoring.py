"""Time anchoring of the direct single-knot evaluators.

The direct evaluators (``pos`` / ``sep`` / ``zpos`` / ``zvel`` / ``rv`` and
their ``_d`` variants) take the transit-centre time ``tc`` as their time
anchor, plus an optional knot offset ``tk`` that has the same meaning and
value as the ``tk`` argument of ``solve2d`` / ``solve3d`` (0.0 = knot at
...
second argument as "the knot time" silently produced wrong orbits.
"""
import numpy as np
import pytest
...
TK = 0.02                  # off-centre knot offset [days]
K_RV = 50.0
...
class TestAbsoluteTimes2d:
    def test_pos_knot_at_transit(self, newton_xy):
    ...
    def test_pos_offcentre_knot(self, newton_xy):
        c = mm2.solve2d(TK, **PARS)
        x, y = mm2.pos(TIMES, TC, P, c, tk=TK)
        assert_allclose(x, newton_xy[0], **ORACLE_TOL)
        assert_allclose(y, newton_xy[1], **ORACLE_TOL)

    def test_sep_offcentre_knot(self, newton_xy):
        c = mm2.solve2d(TK, **PARS)
        d = mm2.sep(TIMES, TC, P, c, tk=TK)
        assert_allclose(d, np.hypot(*newton_xy), **ORACLE_TOL)

    def test_derivative_variants_match_values(self):
        c, dc = mm2.solve2d_d(TK, **PARS)
        x, y = mm2.pos(TIMES, TC, P, c, tk=TK)
        xd, yd, dx, dy = mm2.pos_d(TIMES, TC, P, c, dc, tk=TK)
        assert_allclose(xd, x, rtol=1e-12)
        assert_allclose(yd, y, rtol=1e-12)
        assert dx.shape == (TIMES.size, 7)
        d, dd = mm2.sep_d(TIMES, TC, P, c, dc, tk=TK)
        assert_allclose(d, np.hypot(x, y), rtol=1e-12)
        assert dd.shape == (TIMES.size, 7)


class TestAbsoluteTimes3d:
    def test_pos_zpos_sep_offcentre_knot(self, newton_xyz):
        xn, yn, zn = newton_xyz
        c = mm3.solve3d(TK, **PARS)
        x, y, z = mm3.pos(TIMES, TC, P, c, tk=TK)
        assert_allclose(x, xn, **ORACLE_TOL)
        assert_allclose(y, yn, **ORACLE_TOL)
        assert_allclose(z, zn, **ORACLE_TOL)
        assert_allclose(mm3.zpos(TIMES, TC, P, c, tk=TK), zn, **ORACLE_TOL)
        assert_allclose(mm3.sep(TIMES, TC, P, c, tk=TK), np.hypot(xn, yn), **ORACLE_TOL)

    def test_rv_offcentre_knot(self):
        c = mm3.solve3d(TK, **PARS)
        rv = mm3.rv(TIMES, K_RV, TC, P, PARS["a"], PARS["i"], PARS["e"], c, tk=TK)
        rv_ref = rv_newton_v(TIMES, K_RV, TC, P, PARS["e"], PARS["w"])
        assert_allclose(rv, rv_ref, rtol=0, atol=0.05)

    def test_derivative_variants_match_values(self):
        c, dc = mm3.solve3d_d(TK, **PARS)
        x, y, z = mm3.pos(TIMES, TC, P, c, tk=TK)
        xd, yd, zd, dx, dy, dz = mm3.pos_d(TIMES, TC, P, c, dc, tk=TK)
        assert_allclose((xd, yd, zd), (x, y, z), rtol=1e-12)
        assert dx.shape == (TIMES.size, 7)
        zp, dzp = mm3.zpos_d(TIMES, TC, P, c, dc, tk=TK)
        assert_allclose(zp, z, rtol=1e-12)
        vz, dvz = mm3.zvel_d(TIMES, TC, P, c, dc, tk=TK)
        assert_allclose(vz, mm3.zvel(TIMES, TC, P, c, tk=TK), rtol=1e-12)
        rv, drv = mm3.rv_d(TIMES, K_RV, TC, P, PARS["a"], PARS["i"], PARS["e"], c, dc, tk=TK)
        assert_allclose(rv, mm3.rv(TIMES, K_RV, TC, P, PARS["a"], PARS["i"], PARS["e"], c, tk=TK),
                        rtol=1e-12)
        assert drv.shape == (TIMES.size, 7)


class TestDispatchPaths:
    """The optional tk must work for scalar times and inside @njit callers."""

    def test_scalar_array_parity_with_tk(self):
        c = mm2.solve2d(TK, **PARS)
        d = mm2.sep(TIMES, TC, P, c, tk=TK)
        for j in (0, 5, 10):
            assert_allclose(mm2.sep(float(TIMES[j]), TC, P, c, tk=TK), d[j], rtol=1e-12)
    ...
    def test_njit_caller_default_and_explicit_tk(self):
        ...
        @njit
        def with_tk(t, tc, p, c, tk):
            return sep3(t, tc, p, c, tk)

        c0 = mm3.solve3d(0.0, **PARS)
        ck = mm3.solve3d(TK, **PARS)
        assert_allclose(with_default(TIMES, TC, P, c0), mm3.sep(TIMES, TC, P, c0), rtol=1e-12)
        assert_allclose(with_tk(TIMES, TC, P, ck, TK), mm3.sep(TIMES, TC, P, ck, tk=TK), rtol=1e-12)


class TestKnot2dAbsoluteAnchoring:
    def test_offcentre_knot_far_tc(self, newton_xy):
        k2 = Knot2D(tc=TC, tk=TK, **PARS)
        k2.set_data(TIMES)
