"""Test suite for meepmeep.backends.numba.ts2d.position module.
...
class TestSolveXYO5S:
    """Test the solve_xy_o5s function (multi-point orbit expansion)."""

    def test_output_structure(self, circular_orbit):
        """Test output structure."""
        # NOTE: solve_xy_o5s has broadcasting issue in line 192
        # It tries to assign (2,5) array to (10,) slot without flattening
        # This is a bug in the source code - skip test
        pytest.skip("solve_xy_o5s has broadcasting bug: assigns (2,5) to (10,) without flatten")

    def test_periodic_boundary(self, circular_orbit):
        """Test that first and last points are identical (periodic orbit)."""
        pytest.skip("solve_xy_o5s has broadcasting bug: assigns (2,5) to (10,) without flatten")

    def test_point_spacing(self, circular_orbit):
        """Test that points are evenly spaced."""
        pytest.skip("solve_xy_o5s has broadcasting bug: assigns (2,5) to (10,) without flatten")


class TestXYT15Functions:
    """Test the xy_t15 family of functions (Taylor series evaluation)."""
    ...
    def test_xy_t15_scalar(self, circular_orbit):
        ...
        t = 0.01
        x, y = xy_t15(t, 0.0, circular_orbit["p"], coeffs)
    ...
    def test_xy_t15_accuracy_vs_newton(self, circular_orbit):
        ...
        for t in times:
            ...
            x_taylor, y_taylor = xy_t15(t, 0.0, circular_orbit["p"], coeffs)

            # Newton-Raphson ground truth
            x_newton, y_newton = xy_newton_v(
                np.array([t]), 0.0, circular_orbit["p"],
...
class TestProjectedDistance:
    ...
    def test_pd_t15c_accuracy_vs_newton(self, circular_orbit):
        ...
        d_taylor = np.array([pd_t15c(t, coeffs) for t in times])

        # Newton-Raphson ground truth (times relative to t0=0)
        d_newton = z_newton_v(
            times, 0.0, circular_orbit["p"],
            circular_orbit["a"], circular_orbit["i"],
            circular_orbit["e"], circular_orbit["w"]
        )
...
class TestAccuracyVsNewton:
    ...
    def test_accuracy_various_eccentricities(self, e):
        ...
        for t in times:
            x_taylor, y_taylor = xy_t15(t, 0.0, params["p"], coeffs)
            x_newton, y_newton = xy_newton_v(
    ...
    def test_degradation_far_from_expansion(self, eccentric_orbit):
        ...
        t_near = 0.01
        x_near, y_near = xy_t15(t_near, 0.0, eccentric_orbit["p"], coeffs)
        x_newton_near, y_newton_near = xy_newton_v(
            np.array([t_near]), 0.0, eccentric_orbit["p"],
        ...
        t_far = 0.3
        x_far, y_far = xy_t15(t_far, 0.0, eccentric_orbit["p"], coeffs)
        x_newton_far, y_newton_far = xy_newton_v(
            np.array([t_far]), 0.0, eccentric_orbit["p"],
            eccentric_orbit["a"], eccentric_orbit["i"],
            eccentric_orbit["e"], eccentric_orbit["w"]
        )
