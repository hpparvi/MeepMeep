...
MM_INLINE void pos_c2(REAL t, MM_GLOBAL const REAL *c, REAL *px, REAL *py) {
    *px = taylor5(t, c);
    *py = taylor5(t, c + 5);
}
...
MM_INLINE void pos2(REAL t, REAL tc, REAL p, MM_GLOBAL const REAL *c, REAL te,
                 REAL *px, REAL *py) {
...
MM_INLINE REAL sep_c2(REAL t, MM_GLOBAL const REAL *c) {
    REAL px, py;
    pos_c2(t, c, &px, &py);
...
MM_INLINE REAL sep2(REAL t, REAL tc, REAL p, MM_GLOBAL const REAL *c, REAL te) {
    REAL epoch = floor((t - tc - te + (REAL)0.5 * p) / p);
