...
"""Multi-expansion-point planet (x, y, z) position evaluators with parameter derivatives."""
...
def _pos_ow(t, tpa, p, dt, ep_table, ep_times, coeffs, dcoeffs, dx, dy, dz):
...
@njit(fastmath=True)
def _pos_osd(t, tpa, p, dt, ep_table, ep_times, coeffs, dcoeffs):
...
def _pos_ovd(times, tpa, p, dt, ep_table, ep_times, coeffs, dcoeffs):
    ...
    dzs = zeros((n, 7))
    for j in range(n):
        xs[j], ys[j], zs[j] = _pos_ow(times[j], tpa, p, dt, ep_table, ep_times, coeffs, dcoeffs,
                                      dxs[j], dys[j], dzs[j])
...
def pos_od(t, tpa, p, dt, ep_table, ep_times, coeffs, dcoeffs):
