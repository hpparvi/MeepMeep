...
"""Transit geometry from a single 3D expansion: contact points, bounding box, durations, and minimum separation."""
...
def find_contact_point(k: float, point: int, c: NDArray):
    ...
    c : NDArray, shape (3, 5)
        Taylor coefficient matrix from :func:`solve3d`.
...
def t14(k: float, c: NDArray) -> float:
    ...
    k : float
        Radius ratio.
    c : NDArray
        A (3, 5) Taylor coefficient matrix.
